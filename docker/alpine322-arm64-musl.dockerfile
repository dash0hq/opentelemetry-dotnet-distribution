...
#
# Alpine (musl) arm64 build container for the native tracer library
# (OpenTelemetry.AutoInstrumentation.Native.so).
#
# musl counterpart of ubuntu1604-arm64.dockerfile -- see that file's header
# for the overall rationale (from-scratch, versioned build container built
# from the upstream commit under test as Docker build context). Unlike the
# glibc variants there's no "oldest glibc we must still support" constraint
# driving the base image choice here: musl doesn't carry the same ABI-
# versioning history, so this tracks a current Alpine release rather than
# an old one. Alpine's own clang/cmake/make are new enough (C++17, cmake
# >=3.10) that none of the Xenial file's GitHub-release workarounds apply.
#
