...
- R11. The distro does not carry a non-runtime-support patch on a Dash0 fork branch without a documented upstream PR reference recorded in the branch's R10 metadata file. **The long-lived `dash0/net6plus-support` branch is exempt** — it carries `reason=runtime-support-net6plus` in place of an upstream PR reference and is explicitly not staged for upstream PRs.
...
- F3. **Upstream merges a Dash0-authored PR the distro has been carrying**
  ...
  - **Steps:** The short-lived branch is deleted per R15. The `-distro` build's substitution entry for that module is removed and the module returns to being consumed as an ordinary NuGet package. Canonical-application diff runs across the full R21 runtime matrix to detect telemetry-shape drift; drift is either accepted or reconciled. The `dash0/net6plus-support` branch is unaffected by this flow (it does not close through merges).
...
- F5. **Runtime-support-branch rebase against a new upstream release**
  - **Trigger:** A new upstream release lands; the long-lived `dash0/net6plus-support` branch on `dash0hq/opentelemetry-dotnet-instrumentation` (and, where required, on `dash0hq/opentelemetry-dotnet`) must rebase onto it.
...
- F6. **Retiring `.NET 6` per the runtime-support policy**
  ...
  - **Steps:** Announce the retirement per the policy's downstream-notification path (release notes, N releases in advance). At the cutover release, on the `dash0/net6plus-support` branch: remove `net6.0` from the multi-target `<TargetFrameworks>` lists and clean up `#if NET6_0` guards. Remove `net6.0` from R21's TFM set, from the R16 canonical-application diff matrix, and from the CI test matrix. The `dash0/net6plus-support` branch itself remains for the other extended runtimes it carries. The retirement is recorded in `SUPPORTED-RUNTIMES.md`.
...
- AE4. **.NET 6 support extends past upstream drop**
  ...
  - **Given:** Upstream `opentelemetry-dotnet-instrumentation` release vX drops `net6.0` from its TFM set. The Dash0 fork's `dash0/net6plus-support` branch has been rebased onto vX and carries the necessary multi-target patches to keep `net6.0` (alongside `net7.0` and any other extended runtimes) compiling and instrumenting.
...
- AE5. **Runtime-support-branch rebase conflict blocks release**
  - **Covers:** R9, F5, Q13.
  - **Given:** Upstream vY refactors an internal API on which the `dash0/net6plus-support` branch's patches depend. Rebase of the runtime-support branch onto vY produces conflicts.
...
- Non-runtime-support patches carried on Dash0 fork branches stay rare in practice. The "own as little as possible" principle depends on this being true for the short-lived branches; the `dash0/net6plus-support` branch is a separate, structurally-permanent surface with its own cost model.
...
- Q8. Fork branching model naming: upstream-tracking branch (`dash0-upstream` vs reusing `main`), short-lived feature-branch convention (`dash0/<slug>`), long-lived runtime-support branch name (proposed `dash0/net6plus-support`), and where the branching model is documented so it applies uniformly across all forks.
