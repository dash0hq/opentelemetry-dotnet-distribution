...
// Package aspnetcorenet10 is the net10.0 twin of the aspnetcore scenario. It
...
// there could silently break net10.0 tracing while net6.0 keeps working.
package aspnetcorenet10_test
...
func TestAspNetCoreHttpClientNet10(t *testing.T) {
	sink := otelsink.Start(t)
	ctx := context.Background()

	container := harness.StartInstrumentedApp(t, ctx, sink, harness.AppScenario{
		ExampleDir:  "examples/aspnetcore-httpclient-net10",
		TestdataDir: "test/e2e/testdata/aspnetcore-httpclient-net10",
	...
	assert.True(t, strings.HasPrefix(runtimeVersion, "10."), "expected process.runtime.version to start with \"10.\", got %q", runtimeVersion)
}
